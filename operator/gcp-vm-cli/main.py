...
def run_gcloud(args: list[str], check: bool = True, capture_output: bool = False) -> subprocess.CompletedProcess:
...
def upload_image_to_gcs(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        run_command(
            ["tar", "-czf", str(tar_gz_path), "-C", str(temp_path), "disk.raw"]
        )
...
def get_image_hash(image_path: Path) -> str:
    ...
    logging.warning(f"PCR4 measurement not found in {measurement_path}, falling back to file modification time for hash.")
...
def main(
    ...
    try:
        ...
        if not wait_for_notarizer_service(cvm_ip, 443, operator_cert_path, operator_key_path):
