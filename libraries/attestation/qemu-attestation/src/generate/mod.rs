...
use crate::{QEMUVmAttestationDocument, VTPM_AK_HANDLE};
...
pub struct QEMUVmAttestationDocumentGenerator(tss_esapi::Context);

impl QEMUVmAttestationDocumentGenerator {
...
#[async_trait]
impl AsyncGenerateAttestationDocument for QEMUVmAttestationDocumentGenerator {
    type AttestationDocument = QEMUVmAttestationDocument;

    /// Produce an attestation document attesting to the selected PCR list
    ///
    /// This function should only be called on an QEMU VM.
...
mod test {
    ...
    #[tokio::test]
    #[ignore]
    async fn test_qemu_vm_attestation_document() -> anyhow::Result<()> {
        let pcr_selection_list = PcrSelectionList::builder()
            .with_selection(HashingAlgorithm::Sha256, ALL_SLOTS)
            .build()?;

        let _doc = QEMUVmAttestationDocumentGenerator::new_with_default()?
            .generate_attestation_document(&pcr_selection_list)
            .await?;

        let file = File::create("test_data/qemu_vm_attestation_document.json")?;
