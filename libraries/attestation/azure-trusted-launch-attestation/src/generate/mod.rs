...
use crate::{TrustedLaunchVmAttestationDocument, VTPM_AK_HANDLE};
...
pub struct TrustedLaunchVmAttestationDocumentGenerator(tss_esapi::Context);

impl TrustedLaunchVmAttestationDocumentGenerator {
...
#[async_trait]
impl AsyncGenerateAttestationDocument for TrustedLaunchVmAttestationDocumentGenerator {
    type AttestationDocument = TrustedLaunchVmAttestationDocument;

    /// Produce an attestation document attesting to the selected PCR list
    ///
    /// This function should only be called on an Azure Trusted Launch VM.
    async fn generate_attestation_document(
        ...
        let ak_cert_chain = collateral::get_ak_cert_chain(&mut self.0)?;

        Ok(Self::AttestationDocument {
            quote,
            ak_cert_chain,
        })
    }
...
mod test {
    ...
    use crate::TrustedLaunchVmAttestationDocumentGenerator;
    ...
    #[tokio::test]
    #[ignore]
    async fn test_azure_trusted_launch_vm_attestation_document() -> anyhow::Result<()> {
        let pcr_selection_list = PcrSelectionList::builder()
            .with_selection(HashingAlgorithm::Sha256, ALL_SLOTS)
            .build()?;

        let _doc = TrustedLaunchVmAttestationDocumentGenerator::new_with_default()?
            .generate_attestation_document(&pcr_selection_list)
            .await?;

        let file = File::create("test_data/azure_trusted_launch_vm_attestation_document.json")?;
