use crate::common::constants::TPM2_PCR_SELECT_MAX;
// Copyright 2020 Contributors to the Parsec project.
// SPDX-License-Identifier: Apache-2.0
use crate::common::{HashingAlgorithm, PcrSlot};

use crate::common::error::{Result, WrapperErrorKind};

use log::error;
use std::convert::TryFrom;
use tpm_structs::TpmsPcrSelection;

use super::select_size::PcrSelectSize;
use super::slot_collection::PcrSlotCollection;
// use tpm2_rs_base::TpmsPcrSelection;
...
impl PcrSelection {
    ...
    pub fn merge_exact(&mut self, other: &Self) -> Result<()> {
        ...
        if self.hashing_algorithm != other.hashing_algorithm {
            ...
            return Err(WrapperErrorKind::InvalidParam);
        }
    ...
    pub fn subtract_exact(&mut self, other: &Self) -> Result<()> {
        ...
        if self.hashing_algorithm != other.hashing_algorithm {
            ...
            return Err(WrapperErrorKind::InconsistentParams);
        }
...
impl TryFrom<TpmsPcrSelection<'_>> for PcrSelection {
    type Error = WrapperErrorKind;

    fn try_from(tss_pcr_selection: TpmsPcrSelection) -> Result<Self> {
        // Parse hashing algorithm.
        let hashing_algorithm =
            HashingAlgorithm::try_from(tss_pcr_selection.hash).map_err(|e| {
                error!("Error converting hash to a HashingAlgorithm: {}", e);
        ...
        let size_of_select: u8 = tss_pcr_selection.pcr_select.len().try_into().map_err(|e| {
            error!("Invalid buffer length for pcr_select : {}", e);
            WrapperErrorKind::InvalidParam
        })?;

        let mut octets = [0u8; TPM2_PCR_SELECT_MAX as usize];
        if tss_pcr_selection.pcr_select.len() > TPM2_PCR_SELECT_MAX as usize {
            error!(
                "Invalid buffer length for pcr_select : {}",
                tss_pcr_selection.pcr_select.len()
            );
            return Err(WrapperErrorKind::InvalidParam);
        }
        octets[..tss_pcr_selection.pcr_select.len()].copy_from_slice(&tss_pcr_selection.pcr_select);

        let pcr_slot_collection = PcrSlotCollection::try_from((size_of_select, octets))?;
