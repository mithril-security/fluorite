...
impl MyState {
    async fn get_certificate_info(
        ...
        let public_key_algorithm_oid_entry = oid_registry.get(public_key_algorithm_oid).ok_or(
            anyhow!("Did not find public key algorithm in the OID registry"),
        )?;
    ...
    async fn get_entries(self, domain: Domain) -> anyhow::Result<Vec<Entry>> {
        ...
        let response = self
            .client
            .get(format!("https://crt.sh/json?q={host}"))
            .send()
            .await
