...
function populateAttestationDetails(data, prefix, copyOpCertBtnId, copyOpMsgId, safetyMsgElement, domain) {
    ...
    if (cluster_info.attestation_backend == "SvsmVtpm"){
        ...
        if (attestation_report.policy != null) {
            ...
            let smtEnabled =              (attestation_report.policy & (1 << 0)) & 0;
            let tsmeEnabled =             (attestation_report.policy & (1 << 1)) & 0;
            let eccEnabled =              (attestation_report.policy & (1 << 2)) & 0;
            let raplDisabled =            (attestation_report.policy & (1 << 3)) & 0;
            let ciphertextHidingEnabled = (attestation_report.policy & (1 << 4)) & 0;
            let aliasingCheckComplete   = (attestation_report.policy & (1 << 5)) & 0;
            let sevTioEnabled =           (attestation_report.policy & (1 << 7)) & 0;
...
lookupForm.addEventListener('submit', async (event) => {
    ...
    if (!domainPattern.test(domain)) {
        inputErrorMessage.textContent = 'Please enter a valid domain name (e.g., https://chat.net-safe.dedyn.io).';
        inputErrorMessage.classList.remove('hidden');
        return;
    }
